    ...
    public class MakeAnythingCraftableSettings : ModSettings
    {
        ...
        public void DrawPage(Rect rect)
        {
            ...
            if (curRecipe.productString != null && curRecipe.recipeUsersString.Any() && curRecipe.workAmount > 0)
            {
                ...
                if (!removedRecipeDefs.Any(x => x.defName == curRecipe.defName))
                {
                    var resetButtonRect = new Rect(buttonRect.xMax + 15, firstColumnPos.y, buttonWidth, 32);
                    if (Widgets.ButtonText(resetButtonRect, "MAC.ResetToDefault".Translate()))
                    {
                        modifiedRecipeDefs.RemoveAll(x => x.defName == curRecipe.defName);
                        var originalDef = DefDatabase<RecipeDef>.GetNamed(curRecipe.defName);
                        curRecipe = new RecipeDefExposable(originalDef);
                        curRecipe.ResolveReferences();
                        recipeLabel = "";
                        ApplySettings();
                    }
